...
window.debugCanvas = {
  ...
  elements: () => window.__UNIFIED_CANVAS_STORE__?.getState().element.getAllElements(),
  selected: () => Array.from(window.__UNIFIED_CANVAS_STORE__?.getState().selection.getSelectedIds() || []),
};
