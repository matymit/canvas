...
function LiveAnnouncements(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const label = selectedTool === 'pan' ? 'Pan tool activated' :
                  selectedTool === 'select' ? 'Select tool activated' :
                  `${selectedTool} tool activated`;
    setMsg(label);
  }, [selectedTool]);

  return (
    <div aria-live="polite" style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(1px, 1px, 1px, 1px)' }}>{msg}</div>
  );
}

export default function Canvas(): JSX.Element {
  // Size & DPR
  const { containerRef, width, height, dpr } = useCanvasSizing({ observeDPR: true });
    ...
    const unsub = useUnifiedCanvasStore.subscribe(
      ...
      (snap) => {
        try {
          ...
          // Render nodes first
          nodes.forEach((n) => mindmapRendererRef.current!.renderNode({
            ...n,
          ...
          const nodeMap = new Map(nodes.map((n) => [n.id, { ...n, text: n.data?.text ?? n.text ?? '', style: n.data?.style ?? n.style } ]));
          const getNodeCenter = (id: string) => {
            const n = nodeMap.get(id);
            if (!n) return null;
            return { x: n.x + (n.width || 0) * 0.5, y: n.y + (n.height || 0) * 0.5 };
          };

          // Render edges
          edges.forEach((e) => mindmapRendererRef.current!.renderEdge({
            ...e,
  ...
  useMindmapLiveRouting(stageRef, mindmapRendererRef.current, { throttleMs: 16 });
