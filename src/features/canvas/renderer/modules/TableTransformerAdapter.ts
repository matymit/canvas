...
export function createTableBoundBoxFunc(
  ...
  return (oldBox: Konva.NodeConfig, newBox: Konva.NodeConfig) => {
    // Get current keyboard state
    const shiftKey = window.event ? (window.event as KeyboardEvent).shiftKey : false;
    const altKey = window.event ? (window.event as KeyboardEvent).altKey : false;
    const ctrlKey = window.event ? (window.event as KeyboardEvent).ctrlKey : false;
    ...
    if (shiftKey) {
      ...
      if (widthChange > heightChange) {
        ...
        constrainedHeight = Math.max(minHeight, constrainedWidth / originalAspect);
      } else {
        // Height changed more, lock width to height
        constrainedWidth = Math.max(minWidth, constrainedHeight * originalAspect);
      }
    }
...
export function createTableTransformerHandlers(
  ...
  return {
    onTransformStart: (nodes: Konva.Node[]) => {
 ...
 */
export function getTableTransformerConfig(element: TableElement): Partial<Konva.TransformerConfig> {
  return {
