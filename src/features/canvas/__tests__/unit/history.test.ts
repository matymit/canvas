...
describe('Undo/Redo Batching', () => {
  ...
  it('should commit grouped operations as single history entry', () => {
    ...
    const el1: CanvasElement = { id: '1', type: 'rect', x: 10 };
    const el2: CanvasElement = { id: '2', type: 'circle', x: 20 };
  ...
  it('should be reversible in order without orphaning elements', () => {
    ...
    // Add initial element
    const initialEl: CanvasElement = { id: 'initial', type: 'rect' };
    ...
    // Verify state
    expect(useUnifiedCanvasStore.getState().elementOrder).toEqual(['initial', '1', '2']);
  ...
  it('should maintain selection state across undo/redo', () => {
    ...
    // Undo
    (store as any).undo();
    expect(useUnifiedCanvasStore.getState().element.getById('1')?.x).toBeUndefined(); // back to original
    expect(useUnifiedCanvasStore.getState().selectedElementIds.has('1')).toBe(true); // selection maintained from before batch
