...
class MockKonvaNode {
  ...
  x(val?: number) { if (val !== undefined) this._x = val; return this._x; }
  y(val?: number) { if (val !== undefined) this._y = val; return this._y; }
  scaleX(val?: number) { if (val !== undefined) this._scaleX = val; return this._scaleX; }
  scaleY(val?: number) { if (val !== undefined) this._scaleY = val; return this._scaleY; }
  rotation(val?: number) { if (val !== undefined) this._rotation = val; return this._rotation; }
  id() { return this._id; }
}
...
describe('Geometry Helpers Unit Tests', () => {
  ...
  describe('History Batching Integration', () => {
    it('should begin batch with correct label', () => {
      ...
      expect(mockStore.history.beginBatch).toHaveBeenCalledWith('Custom transform');
    });
    ...
    it('should handle missing history gracefully', () => {
      const storeWithoutHistory = { getElement: vi.fn(), updateElement: vi.fn() };
    ...
    testCases.forEach(({ type, expectedCommit }) => {
      it(`should handle ${type} elements correctly`, () => {
        ...
        if (expectedCommit === 'connector') {
          ...
          expect(mockStore.updateElement).toHaveBeenCalledWith(
            `${type}-1`,
            expect.not.objectContaining({ width: expect.anything(), height: expect.anything() }),
            { pushHistory: true }
          );
        } else {
