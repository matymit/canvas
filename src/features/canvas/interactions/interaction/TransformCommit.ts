...
type Store = {
  ...
  updateElement?: (id: ElementId, patch: Partial<AnyElement>, opts?: { pushHistory?: boolean }) => void;
  history?: { beginBatch?: (label?: string) => void; endBatch?: (commit?: boolean) => void };
};
...
function commitRectLike(el: AnyElement, node: Konva.Node, store: Store) {
  ...
  k.scaleX(1);
  k.scaleY(1);
  store.updateElement?.(el.id, { x: nx, y: ny, width: w, height: h, rotation: rot }, { pushHistory: true });
}
...
function commitTable(el: AnyElement & { colWidths?: number[]; rowHeights?: number[] }, node: Konva.Node, store: Store) {
  const k = node as any;
  const sx = k.scaleX() ?? 1;
  const sy = k.scaleY() ?? 1;
  const nx = k.x();
  const ny = k.y();
  const rot = k.rotation?.() ?? el.rotation ?? 0;
  const wNew = Math.max(1, round((el.width ?? 1) * sx));
  const hNew = Math.max(1, round((el.height ?? 1) * sy));
  const colWidths = (el.colWidths ?? []).map((v) => Math.max(8, round(v * sx)));
  const rowHeights = (el.rowHeights ?? []).map((v) => Math.max(8, round(v * sy)));
  // reset scales to 1
  k.scaleX(1);
  k.scaleY(1);
  store.updateElement?.(
...
function commitConnector(el: AnyElement, node: Konva.Node, store: Store) {
  ...
  const ny = k.y();
  const rot = k.rotation?.() ?? el.rotation ?? 0;
  store.updateElement?.(el.id, { x: nx, y: ny, rotation: rot }, { pushHistory: true });
}

export function commitTransformForNode(node: Konva.Node, deps: TransformCommitDeps) {
...
export function beginTransformBatch(deps: TransformCommitDeps, label = 'transform') {
