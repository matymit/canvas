...
import { memoryUtils, getMemoryManager } from '../utils/performance/MemoryManager';
...
export interface MemoryCleanupUtils {
  ...
  trackTimer: (timerId: number, type?: 'timeout' | 'interval', metadata?: Record<string, any>) => string;
  trackAnimation: (frameId: number, metadata?: Record<string, any>) => string;
  trackCustom: (resource: any, cleanup: () => void, metadata?: Record<string, any>) => string;
 ...
 */
export function useMemoryCleanup(options: UseMemoryCleanupOptions = {}): MemoryCleanupUtils {
  const {
  ...
  const componentIdRef = useRef(`${trackComponentName}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
  ...
  const cleanupResources = useCallback(() => {
    ...
    if (enableDebugLogging && cleanedCount > 0) {
      console.debug(`useMemoryCleanup[${componentIdRef.current}]: Cleaned up ${cleanedCount} resources`);
    }
  ...
  const trackResource = useCallback((trackFn: () => string) => {
    if (isUnmountedRef.current) {
      console.warn(`useMemoryCleanup[${componentIdRef.current}]: Attempted to track resource after unmount`);
      return '';
    }
    ...
    try {
      ...
      console.error(`useMemoryCleanup[${componentIdRef.current}]: Error tracking resource:`, error);
      return '';
    }
  ...
  const utils: MemoryCleanupUtils = {
    trackNode: useCallback((node: Konva.Node, metadata?: Record<string, any>) => {
      return trackResource(() => memoryUtils.trackNode(node, {
    ...
    trackListener: useCallback((
      ...
      return trackResource(() => memoryUtils.trackListener(target, event, listener, options, {
        componentId: componentIdRef.current,
        componentName: trackComponentName,
        ...metadata,
      }));
    ...
    trackTimer: useCallback((timerId: number, type: 'timeout' | 'interval' = 'timeout', metadata?: Record<string, any>) => {
      return trackResource(() => memoryUtils.trackTimer(timerId, type, {
        componentId: componentIdRef.current,
        componentName: trackComponentName,
        ...metadata,
      }));
    ...
    trackAnimation: useCallback((frameId: number, metadata?: Record<string, any>) => {
      return trackResource(() => memoryUtils.trackAnimation(frameId, {
        componentId: componentIdRef.current,
        componentName: trackComponentName,
        ...metadata,
      }));
    ...
    trackCustom: useCallback((resource: any, cleanup: () => void, metadata?: Record<string, any>) => {
    ...
    cleanup: useCallback((resourceId: string) => {
...
export function useTimerCleanup(componentName?: string) {
  ...
  const setTrackedTimeout = useCallback((callback: () => void, delay: number) => {
    ...
    const trackingId = memoryCleanup.trackTimer(timerId, 'timeout');
    return { timerId, trackingId };
  }, [memoryCleanup]);
  
  const setTrackedInterval = useCallback((callback: () => void, delay: number) => {
    const timerId = setInterval(callback, delay);
    const trackingId = memoryCleanup.trackTimer(timerId, 'interval');
    return { timerId, trackingId };
  }, [memoryCleanup]);
