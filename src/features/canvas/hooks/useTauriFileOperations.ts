...
export function useTauriFileOperations(): UseTauriFileOperationsResult {
  ...
  const clear = useUnifiedCanvasStore(state => state.clear);
  ...
  const loadCanvas = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (result.viewport) {
        ...
        if (result.viewport.x !== undefined && result.viewport.y !== undefined) {
   ...
   */
  const saveWithHistory = useCallback(async (description?: string): Promise<string | null> => {
   ...
   */
  const loadWithHistory = useCallback(async (description?: string): Promise<boolean> => {
   ...
   */
  const exportAsImage = useCallback(async (stageElement?: HTMLElement): Promise<string | null> => {
    try {
      // Find stage element if not provided
      const stage = stageElement || document.querySelector('.konvajs-content') as HTMLElement;
  ...
  const performAutoSave = useCallback(async () => {
    try {
      const path = await TauriFileService.autoSaveCanvas(elements, elementOrder, viewport);
      if (path) {
   ...
   * Enable auto-save
   */
  const enableAutoSave = useCallback((intervalMs: number = 60000) => {
        ...
        if (autoSaveData && autoSaveData.elements.size > 0) {
          ...
          if (currentElements.size === 0) {
            ...
            if (autoSaveData.viewport) {
              const setViewport = useUnifiedCanvasStore.getState().viewport;
              if (autoSaveData.viewport.x !== undefined && autoSaveData.viewport.y !== undefined) {
                setViewport.setPan(autoSaveData.viewport.x, autoSaveData.viewport.y);
              }
